...
beta_start = 0.0001    # Noise variance start
beta_end = 0.01         # Final noise variance
image_size = 28
batch_size = 32
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.enc1 = nn.Sequential(nn.Conv2d(1, 16, kernel_size=3, padding=1), nn.ReLU())
        ...
        self.bottleneck1 = nn.Sequential(nn.Conv2d(96, 96, kernel_size=3, padding=1), nn.ReLU())
        self.bottleneck2 = nn.Sequential(nn.Conv2d(96, 64, kernel_size=3, padding=1), nn.ReLU())
        
        # Decoder - spatial back to original
        self.dec3 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1), nn.ReLU())
        self.dec2 = nn.Sequential(nn.Conv2d(32, 16, kernel_size=3, padding=1), nn.ReLU())
        self.dec1 = nn.Sequential(nn.Conv2d(16, 1, kernel_size=3, padding=1))
        
        # Time embedding
        self.time_embed = nn.Sequential(nn.Linear(1, 32), nn.ReLU(), nn.Linear(32, 32))
    ...
    def forward(self, x, t):
        ...
        embed = self.time_embed(t.float().unsqueeze(1)).view(-1, 32, 1, 1)
...
for epoch in range(epochs):
    ...
    for images, _ in dataset_one:
        ...
        loss = loss_fn(predicted_noise, noise) * 10
